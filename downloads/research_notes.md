# 经典机器学习算法：线性回归与逻辑回归 · 资料核查笔记

核查日期：2026-09-14。范围：本科小组 15–20 分钟汇报，主题是经典监督学习中的两个线性基线。全部参考资料来自 Google ML Crash Course、scikit-learn 官方文档与 Stanford CS229；以下教学组织和数值示例为本项目整理。

## 一、建议覆盖范围

把两个算法放在经典机器学习里定位：都是监督学习，都从特征的线性加权出发，都适合作为可解释的基线。主线用同一种输入连接两个任务，例如“根据学习时长等特征预测考试分数”和“预测是否通过的概率”。这些仅是教学情境，不应宣称学习时长对成绩存在已证明的因果效应。

建议正文回答六个问题：预测什么 → 模型怎样表示 → 用什么衡量错误 → 参数怎样学习 → 输出怎样用于决策 → 怎样评价新样本上的表现。线性回归常用于连续数值预测；逻辑回归本次只讲二分类概率建模与分类。

现场演示控制在四个短实验：直线拟合 35 秒、梯度下降 40 秒、Sigmoid 30 秒、阈值 35 秒。高级推导、多分类、正则化路径、ROC/AUC 等放入网页扩展，不挤占正文。

课后第 08 章用三份公开数据复现算法，不占用 18 分钟讲稿：
...
详细步骤、标签定义与不应过度解释的边界见《经典试验与出处.md》。
