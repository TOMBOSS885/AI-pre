...
const C={blue:'#1d4ed8',mint:'#0f766e',red:'#b4233a',ink:'#1b2433',muted:'#5c6576',grid:'#e4ddd0'};
...
function renderBoundary(){const w1=+$('w1').value,w2=+$('w2').value,b=+$('b2').value;for(const [id,v] of [['w1Out',w1],['w2Out',w2],['b2Out',b]])$(id).textContent=v.toFixed(1);const p=chart('boundaryPlot',{xmin:-2.5,xmax:2.5,ymin:-2.5,ymax:2.5,xlabel:'标准化学习时长 x₁',ylabel:'标准化练习完成量 x₂',xticks:[-2,-1,0,1,2],yticks:[-2,-1,0,1,2]});const poly=positivePolygon(w1,w2,b);let body=`<rect x="${p.x(-2.5)}" y="${p.y(2.5)}" width="${p.x(2.5)-p.x(-2.5)}" height="${p.y(-2.5)-p.y(2.5)}" fill="#1d4ed8" fill-opacity=".06"/>`;if(poly.length)body+=`<polygon points="${poly.map(q=>p.x(q[0])+','+p.y(q[1])).join(' ')}" fill="#0f766e" fill-opacity=".16"/>`;if(Math.abs(w2)>1e-12)body+=pathLine(p,x=>-(w1*x+b)/w2,-2.5,2.5,C.ink,2.5);else if(Math.abs(w1)>1e-12){const x=-b/w1;body+=`<line x1="${p.x(x)}" x2="${p.x(x)}" y1="${p.y(-2.5)}" y2="${p.y(2.5)}" stroke="${C.ink}" stroke-width="2.5"/>`;}let wrong=0;for(const [x,y,label] of BOUNDARY_DATA){if((w1*x+w2*y+b>=0?1:0)!==label){wrong++;body+=`<circle cx="${p.x(x)}" cy="${p.y(y)}" r="10" fill="none" stroke="${C.red}" stroke-width="1.5"/>`;}body+=label?dot(p,x,y,C.mint,6):diamond(p,x,y,C.blue,7);}p.finish(body);$('boundaryReadout').textContent=(w1===0&&w2===0?`所有位置的概率都是 ${sigmoid(b).toFixed(3)}，没有普通分割直线。`:`边界：${w1.toFixed(1)}x₁ + ${w2.toFixed(1)}x₂ + ${b.toFixed(1)} = 0。`)+` 模拟样本误分类 ${wrong}/12。`;}
